...
export async function startGoogleRecording(page: Page, botConfig: BotConfig): Promise<void> {
  ...
  if (transcriptionEnabled) {
    ...
    log(`[Node.js] Using Deepgram directly for Google Meet`);
  } else {
  ...
  await page.evaluate(
    async (pageArgs: {
      ...
      // Use BrowserDeepgramService with stubborn mode to enable reconnection on Google Meet
      const deepgramService = transcriptionEnabled
        ? new browserUtils.BrowserDeepgramService({
        ...
        try {
          ...
          (async () => {
            ...
            audioService.findMediaElements(10, 3000).then(async (mediaElements: HTMLMediaElement[]) => {
              ...
              const connectDeepgram = async () => {
                ...
                try {
                  ...
                  const onMessage = (data: any) => {
                    ...
                    if (!deepgramService.isReady() && data["status"] === "SERVER_READY") {
                  ...
                  const onClose = async (event: CloseEvent) => {
                    ...
                    try { deepgramService.setServerReady(false); } catch { }
                    setTimeout(() => {
                      // Best-effort reconnect; BrowserDeepgramService stubborn mode should also help
                      connectDeepgram().catch(() => { });
                    }, 2000);
                  };
                  ...
                  await deepgramService.connectToDeepgram(
                  ...
                  setTimeout(() => { connectDeepgram().catch(() => { }); }, 2000);
                }
              };
              return await connectDeepgram();
            }).then(() => {
              ...
              const initializeGoogleSpeakerDetection = (deepgramService: any, audioService: any, botConfigData: any) => {
              ...
              if (!degradedNoMedia && transcriptionEnabled && deepgramService) {
                initializeGoogleSpeakerDetection(deepgramService, audioService, botConfigData);
              }
              ...
              const setupGoogleMeetingMonitoring = (botConfigData: any, audioService: any, deepgramService: any, resolve: any) => {
              ...
              setupGoogleMeetingMonitoring(botConfigData, audioService, deepgramService, resolve);
            }).catch((err: any) => {
              reject(err);
            });
          })(); // Close async IIFE
