...
export default function MeetingPage() {
    ...
    useEffect(() => {
        const loadMeeting = async () => {
            try {
                ...
                setTags(data.tags || []);
                setScratchpad(data.scratchpad_notes || '');

                if (data.transcript) {
                    setSummary(data.transcript.summary || data.transcript.cleaned_text || data.transcript.raw_text || '');

                    const segs = data.transcript.action_items;
    ...
    // ─── Auto-save utility ─────────────────────────────────────────────────────────────
    const patchMeeting = async (updates: any) => {
        setSaving(true);
        try {
    ...
    useEffect(() => {
        ...
        const timeout = setTimeout(async () => {
            patchMeeting({ scratchpad_notes: scratchpad });
        }, 1500);
                ...
                <div className={styles.tasksList}>
                    {tasks.map(task => (
                        <div key={task.id} className={`${styles.taskItem} ${task.done ? styles.taskDone : ''}`}>
                            ...
                            <button className={styles.taskDelete} onClick={() => deleteTask(task.id)}>&times;</button>
                        </div>
                    ))}
                </div>
            ) : (
        ...
        <div className={styles.page}>
            ...
            <div className={styles.topBar}>
                <div className={styles.breadcrumb}>
                    <Link href="/dashboard/meetings" className={styles.breadcrumbLink}>Meetings</Link>
                ...
                <div className={styles.topActions}>
                    ...
                    <button className={styles.moreBtn}>&vellip;</button>
                </div>
            </div>
                        ...
                        <span className={styles.metaValue}>
                            {tags.map((tag, i) => (
                                <span key={i} className={styles.tagChip}>
                                    {tag}
                                    <button className={styles.tagRemove} onClick={() => removeTag(i)}>&times;</button>
                                </span>
                            ))}
                            {showTagInput ? (
                                <input
